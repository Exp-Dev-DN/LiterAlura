...
import br.com.literalura.service.CatalogoService; // Importe o novo serviço
...
public class Principal {
    ...
    private void buscarLivroWeb() {
        ...
        try {
            ...
            Optional<DtoLivro> dadosLivroOpt = dadosBusca.resultados().stream()
                    .filter(l -> l.titulo().equalsIgnoreCase(nomeLivro))
                    .findFirst();

            if (dadosLivroOpt.isPresent()) {
                DtoLivro dadosLivro = dadosLivroOpt.get();
                Autor autor;
                if (!dadosLivro.autores().isEmpty()) {
                    DtoAutor dadosAutor = dadosLivro.autores().get(0);
